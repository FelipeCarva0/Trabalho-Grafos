...
//gera a imagem nova definindo uma cor aleatoria para cada conjunto disjunto do grafo final-----------------FELIPE MUDAR COR
...
Mat MST::segment(){
    ...
    float limiar = 30.0f;// define o valor de corte para a diferença de cores

    // Passo 2: Para cada aresta, verificar se os vértices pertencem a componentes diferentes, e os une caso sim.
...
Mat MST::renderSegmentsByMeanColor(DisjointSet& ds, int width, int height) {
    ...
    // Passo 3: detectar bordas — pixel é borda se algum vizinho pertence a componente diferente
    int dx[] = {1, -1, 0, 0};
    int dy[] = {0,  0, 1, -1};
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            for (int d = 0; d < 4; d++) {
                ...
                if (ds.find(ny * width + nx) != root) {
                    result.at<Vec3b>(y, x) = Vec3b(0, 0, 0); // borda preta
                    break;
                }
            }
        }
